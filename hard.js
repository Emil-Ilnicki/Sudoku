 ...
 *
 * @Modified by Harasees Singh Gill
 */
var Sudoku = ( function ( $ ){
    var _instance, _game,
        /**
    ...
    Game.prototype = {
        ...
        resetGame: function() {
            ...
            this.resetValidationMatrices();
                for (var length = 0; length < 62; length++){
                    var row = parseInt(Math.floor(Math.random() * 9));
                    var col = parseInt(Math.floor(Math.random() * 9));
                            // Reset GUI inputs
                        if (this.$cellMatrix[row][col].val('')){
                            length = length
                        }else{
